...
public class MainGame : MonoBehaviour {
	...
	void Update () {
		...
		if(mode==state.Load){
			...
			*/
			...
			if (c.transform.localScale.x < RingSize) {
				...
				Arrow.transform.Rotate (new Vector3 (0f, 0f, -9f), Space.Self);
				if (c.transform.localScale.x >= RingSize) {
					//InvokeRepeating("ShowReady",0f,0.5f);
					StartCoroutine (FadeInReady());
				}
			}
		}
	...
	public IEnumerator FadeInReady(){
		Ready.SetActive (true);
		SpriteRenderer i = Ready.GetComponent<SpriteRenderer> ();
